...
class AvatarForcingRunner:
    ...
    def generate(
        ...
        msk = torch.zeros_like(
            img_lat.repeat(1, 1, self.num_output_frames + 20, 1, 1)[:, :1]
        )
        image_cat = img_lat.repeat(1, 1, self.num_output_frames + 20, 1, 1)
        msk[:, :, 1:] = 1
        ...
        sampled_noise = torch.randn(
            (num_samples, self.num_output_frames - 1, 16, h, w),
            device=device, dtype=self.dtype,
        )
