...
class UnifiedMoshiAvatarPipeline:
    ...
    def run(
        ...
        all_audio_embs:  list[torch.Tensor] = []

        print("[UnifiedPipeline] Running Moshi …")
        for step in self.moshi.run(in_pcm_3d):
            all_pcm_chunks.append(step.pcm_chunk)         # (1, frame_size)

            # Push one step's 8 response tokens to bridge
            audio_emb = self.bridge.push_tokens(step.audio_tokens)  # (1, 8)
            if audio_emb is not None:
                all_audio_embs.append(audio_emb)
                print(f"[UnifiedPipeline]   bridge block {len(all_audio_embs):2d}: "
                      f"{audio_emb.shape}")

        # Flush partial trailing block
        final_emb = self.bridge.flush()
        if final_emb is not None:
            all_audio_embs.append(final_emb)
            print(f"[UnifiedPipeline]   bridge block {len(all_audio_embs):2d} (flush): "
                  f"{final_emb.shape}")

        n_steps = len(all_pcm_chunks)
        print(f"[UnifiedPipeline] Moshi done: {n_steps} steps | "
              f"{len(all_audio_embs)} bridge blocks")

        if not all_audio_embs:
            min_secs = self.bridge.mimi_frames_needed / 12.5
            raise RuntimeError(
                f"No bridge output — audio too short. "
                f"Need ≥ {min_secs:.1f}s, got {duration:.1f}s."
            )

        # ── 3. Use the first bridge block for video generation ────────────────
        # (teacher_len+1, 10752) — same shape as batch['audio_emb'] in dataset.py
        audio_emb_for_video = all_audio_embs[0]
