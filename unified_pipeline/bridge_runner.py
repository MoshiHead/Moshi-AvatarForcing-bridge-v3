...
class BridgeRunner:
    ...
    def push_tokens(self, audio_tokens: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        return self._run_bridge(tokens_batch)
    ...
    @torch.no_grad()
    def _run_bridge(self, tokens: torch.Tensor) -> torch.Tensor:
        """
        ...
        Returns
        -------
        audio_emb : (teacher_len + 1, 10752) float32
        ...
        # Trim or pad to exactly teacher_len + 1
        target_len = self.teacher_len + 1
        ...
        return audio_emb.cpu()    # (teacher_len+1, 10752)
